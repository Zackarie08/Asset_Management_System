/* ============================================================
   subscriptions_unified.js
   ONE unified Subscriptions module covering M365 · Globe · Other
   Replaces: subscriptions.js (refactored in place)
   ============================================================ */
...
/* ──────────────────────────────────────────────────────────
   ...
   Fetches M365 + Globe + Subscriptions, renders one table
────────────────────────────────────────────────────────── */
...
async function renderSubscriptionsUnified() {
  ...
  const totalCost = rows.filter(r => r.status === "Active" && r.cost).reduce((s, r) => s + Number(r.cost), 0);
...
async function renderM365() {
  try {
    ...
    if (tbody) {
      ...
      const expEl = document.getElementById("m365-exp-ct");
      if (expEl) expEl.textContent = `${expiredCount} expired`;
      const ctEl = document.getElementById("m365-ct");
      if (ctEl) ctEl.textContent = `${data.length} licenses`;
    }
...
function saveM365() {
  ...
  const payload = { assigned_email: email, license_type: type, category: cat, monthly_cost: cost,
    start_date: start || null, expiry_date: expiry || null, renewal_date: renewal || null, status: "Active", remarks };
...
function confirmDeleteM365() {
  fetch(`${API_URL}/api/m365/${deleteM365Id}`, { method: "DELETE" })
    ...
    .then(() => { showToast("License deleted", "t-warning"); addLog("DELETE", "M365 LICENSE", `Deleted M365 license #${deleteM365Id}`, null); closeM("m-confirm-m365-del"); closeDP(); renderM365(); })
...
async function renderGlobe() {
  try {
    ...
    if (tbody) {
      ...
      const renewEl = document.getElementById("globe-renew-ct");
      if (renewEl) renewEl.textContent = `${renewSoon} renewing soon`;
      const ctEl = document.getElementById("globe-ct");
      if (ctEl) ctEl.textContent = `${data.length} plans`;
    }
    renderSubscriptionsUnified();
...
async function dpGlobe(id) {
  ...
  } catch (err) { console.error("dpGlobe:", err); showToast("Failed to load Globe plan", "t-error"); }
}
...
function saveGlobe() {
  ...
  if (mobile && !mobilePattern.test(mobile)) { showToast("Invalid mobile format (e.g. 0917-123-4567)", "t-error"); return; }
...
function confirmDeleteGlobe() {
  fetch(`${API_URL}/api/globe/${deleteGlobeId}`, { method: "DELETE" })
    .then(r => { if (!r.ok) return r.json().then(e => { throw new Error(e.error); }); })
    .then(() => { showToast("Plan deleted", "t-warning"); addLog("DELETE", "GLOBE PLAN", `Deleted Globe plan #${deleteGlobeId}`, null); closeM("m-confirm-globe-del"); closeDP(); renderGlobe(); })
    .catch(err => showToast(err.message || "Delete failed", "t-error"));
}
...
async function dpSubscriptions(id) {
  ...
  } catch (err) { console.error("dpSubscriptions:", err); showToast("Failed to load subscription", "t-error"); }
}
...
function saveSubscription() {
  ...
  const payload = { subscription_name: name, category, supplier: supplier || null, assigned_to: assigned || null,
    monthly_cost: cost, billing_cycle: cycle, start_date: start, expiry_date: expiry, status, remarks };
...
function confirmDeleteSubscription() {
  fetch(`${API_URL}/api/subscriptions/${deleteSubId}`, { method: "DELETE" })
    .then(r => { if (!r.ok) return r.json().then(e => { throw new Error(e.error); }); })
    .then(() => { showToast("Subscription deleted", "t-warning"); addLog("DELETE", "SUBSCRIPTION", `Deleted subscription #${deleteSubId}`, null); closeM("m-confirm-sub-del"); closeDP(); renderSubscriptions(); })
    .catch(err => showToast(err.message || "Delete failed", "t-error"));
}

/* ──────────────────────────────────────────────────────────
   ATTACHMENT SYSTEM  (improved — drag & drop, collapsible)
────────────────────────────────────────────────────────── */

// FILE SIZE LIMIT (modifiable)
...
async function attachmentPanel(module, recordId, containerId) {
  ...
  container.innerHTML = `
    <div class="dp-section-hd att-toggle" onclick="toggleAttPanel('${containerId}')" style="cursor:pointer">
      📎 Attachments <span id="att-arrow-${containerId}">▼</span>
    </div>
  ...
  try {
    ...
    document.getElementById(`att-body-${containerId}`).innerHTML =
      `<div style="color:var(--red-500);font-size:12px">Failed to load attachments</div>`;
  }
}
  ...
  const fileRows = files.length
    ? files.map(f => `
        <div style="display:flex;align-items:center;gap:8px;padding:8px 0;border-bottom:1px solid var(--slate-100)">
          <span style="font-size:16px">${_attIcon(f.file_type)}</span>
          <span style="flex:1;font-size:12.5px;font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="${_esc(f.file_name)}">${_esc(f.file_name)}</span>
          ...
          <button class="btn btn-xs btn-red" onclick="deleteAttachment(${f.attachment_id},'${module}',${recordId},'${containerId}')">✕</button>
        </div>`).join("")
    : `<div style="color:var(--slate-400);font-size:12px;padding:8px 0">No attachments yet.</div>`;

  document.getElementById(`att-body-${containerId}`).innerHTML = `
    <div id="${inputId}-dropzone"
      style="border:1.5px dashed var(--slate-300);border-radius:var(--radius-sm);padding:14px;text-align:center;margin-bottom:10px;cursor:pointer;transition:background 0.2s"
      ...
      <div style="font-size:12px;color:var(--slate-400)">📎 Drag & drop or click to upload</div>
      <div style="font-size:11px;color:var(--slate-300);margin-top:3px">Max ${ATTACHMENT_MAX_MB}MB</div>
    </div>
    <input type="file" id="${inputId}" style="display:none" onchange="_uploadFromInput('${module}',${recordId},'${inputId}','${containerId}')"/>
    ...
    try {
      const res = await fetch(`${API_URL}/api/attachments`, {
        ...
        body: JSON.stringify({
          ...
          file_name: file.name, file_url: reader.result,
          file_type: file.type,
