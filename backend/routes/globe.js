// backend/routes/globe.js — REFACTORED
// Changes:
//   • GET /api/globe/:id  (was missing)
//   • Auto-status computation (Active / For Renewal / Inactive)
//   • monthly_cost normalised
...
/**
 * Compute Globe plan status from renewal_date + stored status.
 * Stored status "Inactive" is always respected.
 * Otherwise: if renewal ≤ 30 days → "For Renewal", else "Active".
 */
function computeGlobeStatus(renewalDate, storedStatus) {
  ...
  if (daysLeft < 0)  return "Inactive";       // past renewal with no action
...
router.get("/", async (req, res) => {
  try {
    ...
    const rows = result.rows.map(r => ({
      ...r,
      computed_status: computeGlobeStatus(r.renewal_date, r.status),
    }));
